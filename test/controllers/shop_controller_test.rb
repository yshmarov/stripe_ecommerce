...
class ShopControllerTest < ActionDispatch::IntegrationTest
  test "add to cart" do
    ...
    assert_equal price.stripe_price["unit_amount"], order.total_amount
    assert_equal price.stripe_price["unit_amount"], order.order_items.first.total_amount
    assert_equal price.stripe_price["unit_amount"], order.order_items.first.unit_amount
    ...
    total = price.stripe_price["unit_amount"] + price2.stripe_price["unit_amount"]
    assert_equal total, order.total_amount
    assert_equal price.stripe_price["unit_amount"], order.order_items.first.total_amount
    assert_equal price.stripe_price["unit_amount"], order.order_items.first.unit_amount
    assert_equal price2.stripe_price["unit_amount"], order.order_items.second.unit_amount
    assert_equal price2.stripe_price["unit_amount"], order.order_items.second.total_amount
    ...
    total = price.stripe_price["unit_amount"] + (price2.stripe_price["unit_amount"] * 2)
    assert_equal total, order.total_amount
    assert_equal price.stripe_price["unit_amount"], order.order_items.first.total_amount
    assert_equal price.stripe_price["unit_amount"], order.order_items.first.unit_amount
    assert_equal price2.stripe_price["unit_amount"], order.order_items.second.unit_amount
